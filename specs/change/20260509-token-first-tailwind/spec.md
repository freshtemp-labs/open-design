...
- `specs/change/20260509-token-first-tailwind/token.md` - Tailwind color/radius/shadow token vocabulary, existing CSS variable mapping, native Tailwind primitive decision, and guardrail target。
...
- 决策：项目自定义 Tailwind token 覆盖颜色、radius 和 shadow；radius/shadow utilities 解析到现有 CSS variables，确保卡片、弹窗、模态和暗主题阴影保持当前视觉；font、spacing 和 typography scale 使用 Tailwind 原生 utilities。Source: `specs/change/20260509-token-first-tailwind/token.md`
...
流程：
  ...
  将 `apps/web/postcss.config.mjs` 加入 residual JavaScript guard 精确 allowlist，并记录 PostCSS/Tailwind 只能加载该兼容配置入口的理由。
...
- Tailwind class vocabulary 使用项目 token 名称，例如 `bg-bg`、`bg-panel`、`bg-subtle`、`text-text`、`text-muted`、`border-border`、`text-accent`、`bg-accent`、`text-danger`、`bg-success-surface`、`rounded-card` 和 `shadow-token-sm`；字体、间距和字号使用 Tailwind 原生 utilities，例如 `font-mono`、`gap-3` 和 `text-sm`。
...
- [ ] Step 2: 把 Open Design tokens 暴露为 Tailwind utilities
  - [ ] Substep 2.1 Implement: 为 colors、core semantic status、radius 和 shadow tokens 添加 CSS-first `@theme` aliases；font、spacing 和 typography scale 使用 Tailwind 原生 utilities。
...
- [ ] Step 5: 全量迁移现有 TSX class
  ...
  - [ ] Substep 5.3 Implement: 对当前依赖 `--radius*` 和 `--shadow*` 的卡片、popover、modal、输入与按钮样式，使用 `rounded-card`、`rounded-panel`、`rounded-token-pill`、`shadow-token-sm`、`shadow-token-md` 等变量-backed Tailwind utilities，保持暗主题 shadow override 和现有圆角比例。
